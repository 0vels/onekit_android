        ...
        switch (requestCode){
            case 1:
                if (grantResults.length > 0){
                    ArrayList<String> deniedPermissons = new ArrayList<>();  //被拒绝的权限集合
                    for (int i = 0; i < grantResults.length;i++){
                        int grantResult = grantResults[i];
                        String permisson = permissions[i];
                        if (grantResult != PackageManager.PERMISSION_GRANTED){
                            deniedPermissons.add(permisson);
                        }
                    }
                    if (deniedPermissons.isEmpty()){
                        mPermissionListener.onGranted();
                    }else {
                        mPermissionListener.onDenied(deniedPermissons);
                    }
                }
                break;
            default:
